...
|  |  |
| --- | --- |
| SDK display name | Trader API |
| Root package | `trader_api` |
| Distribution name | `trader-api` |
...
Alternatively, scope it — `with TraderApiClient(...) as client:` closes the pool on exit.
...
Alternatively, scope it — `async with AsyncTraderApiClient(...) as client:` closes the pool on exit.

`AsyncClient` (`trader_api/async_client.py`) mirrors `Client` method for method, each endpoint method a coroutine. It takes the same keywords, except that each client accepts only its own transport and — where the **Async Type** column differs — only its own flavor.

`Client` and `AsyncClient` are aliases of `TraderApiClient` and `AsyncTraderApiClient` — the names tracebacks and `repr()` show; all four import from the root.
...
Every API group is a property on the client (e.g. `client.account_activities`). Every constructor argument is optional and keyword-only. Sources: `trader_api/client.py`, `trader_api/async_client.py`:
...
| `api_secret` | `str \| None` | `str \| None` | `None` |

The types those columns name — where each imports from and, for a credentials dict, its keys:

| Type | Import from | Shape |
| --- | --- | --- |
| `Environment` | `trader_api.server` | `Literal` of the Environments table's names |
| `HttpClient` | `trader_api.core` | protocol — `send(request: HttpRequest) -> HttpResponse` · `close()` |
| `AsyncHttpClient` | `trader_api.core` | protocol — `async send(request: HttpRequest) -> HttpResponse` · `async aclose()` |
...
- **Case A — typed error.** The operation documents at least one error status, so `trader_api/errors/` declares a union alias over the bodies those statuses map to — `RawError` is always its last arm, for any undocumented status — and `.error` is annotated with that alias. Narrow it with `isinstance`. The operation blocks name the alias and the status each arm maps from.
- **Case B — raw error.** The operation documents no error status; `.error` is `RawError` (`trader_api/core/results.py`): `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse`.

Core runtime types (`trader_api/core/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
| --- | --- | --- |
| `ApiError` — raised by every parsed call; `.error` is a Case A alias from `trader_api/errors/` or `RawError` | `error: E` · `status_code: int` · `response: HttpResponse` | `trader_api/core/exceptions.py` |
| `ApiResult[T, E]` — returned by every raw call; the `Success[T] \| Failure[E]` union | `payload: T` (on `Success`) · `error: E` (on `Failure`) · `response: HttpResponse` (on both) | `trader_api/core/results.py` |
| `RawError` | `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse` | `trader_api/core/results.py` |
...
```python
from trader_api.core import ApiError, RawError

try:
    response = client.orders.delete_all_orders()
except ApiError as e:
    # Case A — typed error: e.error is DeleteAllOrdersErrorBody
    if isinstance(e.error, RawError):
        # Any other error status
        print(e.status_code, e.error.text())
```

**Raw (`.with_raw_response`) variants: present on every operation** — the same call returns `ApiResult` instead of raising, with the same body on `Failure.error`. Of **29 operations**, **4 are Case A (typed)** and **25 are Case B (raw)**.

---

## Operations — by controller (9 pages, 29 operations)
...
| **A trailing `request_options`** — keyword-only and optional, for per-call overrides such as a timeout or extra headers; every signature ends with it | here (`trader_api/core/request_options.py`) |
...
| Controller | Ops | Page |
| --- | --- | --- |
| `client.account_activities` | 2 | [map/operations/account_activities.md](map/operations/account_activities.md) |
| `client.account_configurations_api` | 2 | [map/operations/account_configurations_api.md](map/operations/account_configurations_api.md) |
| `client.accounts` | 1 | [map/operations/accounts.md](map/operations/accounts.md) |
| `client.calendar_api` | 1 | [map/operations/calendar_api.md](map/operations/calendar_api.md) |
| `client.clock_api` | 1 | [map/operations/clock_api.md](map/operations/clock_api.md) |
| `client.orders` | 6 | [map/operations/orders.md](map/operations/orders.md) |
| `client.portfolio_history_api` | 1 | [map/operations/portfolio_history_api.md](map/operations/portfolio_history_api.md) |
| `client.positions` | 4 | [map/operations/positions.md](map/operations/positions.md) |
| `client.watchlists` | 11 | [map/operations/watchlists.md](map/operations/watchlists.md) |

---

## Models — where they live, how to build them

**Shapes live only in the source.** Every module under `trader_api/models/` declares one type plus its input companion, and every module under `trader_api/errors/` one alias plus the mapper that builds it; no two share a name. Take a type's module from the operation's **Type sources** table. When no retrieved chunk names it, the module is the type name in snake_case under the kind's directory below (`Account` ↔ `account.py`; an error alias drops its `Body` suffix: `DeleteAllOpenPositionsErrorBody` ↔ `delete_all_open_positions_error.py`). Never grep for a type.

| Group | Count | Directory (module = `<type_name>.py`) |
| --- | --- | --- |
| Models (`SdkBaseModel` pydantic classes) | 16 | `trader_api/models/` |
| Enums (`Enum` over `str`) — Python member names + wire values | 14 | `trader_api/models/enums/` |
| Unions (plain) — `TypeAlias` over the arms | 2 | `trader_api/models/unions/` |
| Error aliases (one per Case A operation) | 4 | `trader_api/errors/` |
...
Every model, enum and union also has an **input companion**, exported beside it from the same package (`Account` ↔ `AccountDict`). Wherever a signature names the companion you may pass either the model instance or a plain dict with the same keys, whichever reads better at the call site. An enum is a real `Enum` subclass over `str`; its companion is spelled `<Name>OrStr` or `<Name>OrInt` (`AccountStatus` ↔ `AccountStatusOrStr`) and additionally accepts a wire value this SDK version does not know. A union is a `TypeAlias` over its arms.

Import paths by content type (`from <package> import <Name>`):

| Contents | Import from |
| --- | --- |
| Client (root) | `trader_api` |
| Operation controllers | `trader_api.apis` |
| Models | `trader_api.models` |
| Enums | `trader_api.models.enums` |
| Unions | `trader_api.models.unions`, `trader_api.models` |
| Error aliases | `trader_api.errors` |
| Core runtime (`ApiError`, `ApiResult`, `RawError`, …) | `trader_api.core` |

---

## Servers & auth

**API key (header `APCA-API-KEY-ID`).** Pass `api_key="<api_key>"`; sent as the `APCA-API-KEY-ID` request header.

**API key (header `APCA-API-SECRET-KEY`).** Pass `api_secret="<api_key>"`; sent as the `APCA-API-SECRET-KEY` request header.
...
**Environments.** `environment=` selects the target environment (`trader_api/server/environment.py`); this SDK's one server (`trader_api/server/server_config.py`) has a base URL per environment:

| Environment | Base URL | Hosting | Override point |
| --- | --- | --- | --- |
| `"paper"` *(default)* | `https://paper-api.alpaca.markets` | Paper trading — simulated money. Safe default. | `base_url="https://…"` |
| `"live"` | `https://api.alpaca.markets` | Live trading — REAL money. | `base_url="https://…"` |
